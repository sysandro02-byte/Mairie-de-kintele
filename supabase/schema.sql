...
-- Administrateur initial LoukaTech : remplaçable ensuite.
insert into public.admin_allowlist(email)
values ('contacts@loukatech.com')
on conflict (email) do nothing;

-- Bootstrap sécurisé : seuls les e-mails explicitement autorisés deviennent admins.
...
begin
  if exists (
    ...
    insert into public.admin_users(user_id, email, full_name, active)
...
insert into public.hero_slides(title, subtitle, image_url, button_label, button_href, sort_order, active) values
...
insert into public.services(icon,title,description,href,sort_order,active) values
...
insert into public.document_types(title,description,requirements,fee_text,processing_time,sort_order,active) values
...
insert into public.news(category,title,excerpt,body,image_url,published,published_at) values
...
grant select on public.site_content, public.hero_slides, public.services, public.news, public.document_types, public.media_library to anon, authenticated;
grant insert on public.document_requests to anon, authenticated;
grant select on public.admin_users to authenticated;
grant select, insert, update, delete on public.site_content, public.hero_slides, public.services, public.news, public.document_types, public.media_library, public.document_requests to authenticated;

create policy "public read site content" on public.site_content for select to anon, authenticated using (true);
create policy "public read active slides" on public.hero_slides for select to anon, authenticated using (active = true or exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active));
create policy "public read active services" on public.services for select to anon, authenticated using (active = true or exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active));
create policy "public read published news" on public.news for select to anon, authenticated using (published = true or exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active));
create policy "public read active documents" on public.document_types for select to anon, authenticated using (active = true or exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active));
create policy "public read media" on public.media_library for select to anon, authenticated using (true);

create policy "user can see own admin row" on public.admin_users for select to authenticated using ((select auth.uid()) = user_id);

create policy "public can create request" on public.document_requests
for insert to anon, authenticated
with check (length(trim(full_name)) >= 2 and length(trim(email)) >= 5 and length(trim(phone)) >= 5);

-- Admin CRUD policies
create policy "admin manage site content" on public.site_content for all to authenticated
using (exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active))
with check (exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active));

create policy "admin manage slides" on public.hero_slides for all to authenticated
using (exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active))
with check (exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active));
...
create policy "admin manage news" on public.news for all to authenticated
using (exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active))
with check (exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active));

create policy "admin manage documents" on public.document_types for all to authenticated
using (exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active))
with check (exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active));

create policy "admin read requests" on public.document_requests for select to authenticated
using (exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active));

create policy "admin update requests" on public.document_requests for update to authenticated
using (exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active))
with check (exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active));

create policy "admin delete requests" on public.document_requests for delete to authenticated
using (exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active));

create policy "admin manage media metadata" on public.media_library for all to authenticated
using (exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active))
with check (exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active));
...
with check (
  ...
  and exists (select 1 from public.admin_users a where a.user_id = (select auth.uid()) and a.active)
);
